...
export function publicGroup(group, token) {
  return { name: group.name, slug: group.slug, owner: isAdmin(group, token), protected: !!group.password, closed: !!group.closed,
    ...
    events: group.events.map(({ creatorToken, ...event }) => ({ ...event, canCancel: isAdmin(group, token) || creatorToken === token, participants: event.participants.map((participant, index) => ({ name: participant.name, mine: participant.token === token, waiting: index >= event.capacity })), comments: (event.comments || []).map(comment => ({ id: comment.id, name: comment.name, text: comment.text, createdAt: comment.createdAt, mine: comment.token === token })) })) };
}
