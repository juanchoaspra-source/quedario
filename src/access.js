export function migrate(group) {
  ...
  for (const p of [{ token: group.owner, name: 'Creador del grupo' }, ...group.events.flatMap(e => e.participants)]) {
    if (!group.members.some(m => m.token === p.token)) group.members.push({ id: crypto.randomUUID(), token: p.token, name: p.name, version: -1 });
  }
  return group;
}
export const isAdmin = (group, token) => (group.admins || [group.owner]).includes(token);
export const canRead = (group, token) => isAdmin(group, token) || (!group.password) || group.members.some(m => m.token === token && m.version === group.accessVersion);
